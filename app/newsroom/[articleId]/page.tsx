    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-gray-50 py-3">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 md:px-8">
          <nav className="flex items-center space-x-1 sm:space-x-2 text-xs text-gray-600">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 md:px-8">
          <div className="mb-6 sm:mb-8">
            <div className="flex flex-wrap items-center gap-2 sm:gap-4 mb-4 sm:mb-6">
              <span className={`px-2 py-1 rounded-full text-white text-xs font-semibold uppercase tracking-wide ${getCategoryColor(article.category)}`}>
                {article.category}
              </span>
              <span className="text-gray-500 hidden sm:inline">•</span>
              <span className="text-xs sm:text-sm text-gray-600">{formatDate(article.publishedAt)}</span>
              <span className="text-gray-500 hidden sm:inline">•</span>
              <span className="text-xs sm:text-sm text-gray-600">{formatTime(article.publishedAt)}</span>
              <span className="text-gray-500 hidden sm:inline">•</span>
              <span className="text-xs sm:text-sm text-gray-600">{article.readTime} min read</span>
            ...
            <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-credera-dark mb-4 sm:mb-6 leading-tight">
              {article.title}
            </h1>
            ...
            <div className="flex items-center space-x-3">
              <div className="relative w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full overflow-hidden">
                <Image
              ...
              <div>
                <p className="font-semibold text-credera-dark text-xs sm:text-sm md:text-base">{article.author.name}</p>
      ...
      <div className="pb-12 sm:pb-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 md:px-8">
          <div className="prose prose-sm sm:prose-base md:prose-lg max-w-none">
            {content.split('\n\n').map((paragraph, index) => (
              <p key={index} className="text-gray-700 leading-relaxed mb-4 sm:mb-6 text-xs sm:text-sm md:text-base">
                {paragraph}
              </p>
            ))}
          </div>
          ...
          {article.tags.length > 0 && (
            <div className="mt-8 sm:mt-10 md:mt-12 pt-6 sm:pt-8 border-t border-gray-200">
              <h3 className="text-base sm:text-lg font-semibold text-credera-dark mb-3 sm:mb-4">Tags</h3>
          ...
          <div className="mt-8 sm:mt-10 md:mt-12 pt-6 sm:pt-8 border-t border-gray-200">
            <Link
              href="/newsroom"
              className="inline-flex items-center text-credera-red hover:text-red-600 font-semibold transition-colors text-xs sm:text-sm"
            >
